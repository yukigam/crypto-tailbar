...
export async function setProfilePhoto(imageUrl) {
  if (!PAGE_ID || !PAGE_ACCESS_TOKEN) {
    return { error: 'Facebook not configured' };
  }
  const uploadRes = await fetch(`https://graph.facebook.com/v22.0/${PAGE_ID}/photos`, {
  ...
  const picRes = await fetch(`https://graph.facebook.com/v22.0/${PAGE_ID}/picture`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ photo_id: uploadData.id, access_token: PAGE_ACCESS_TOKEN }),
  });
  return picRes.json();
}
