...
export function mapSanityPost(p, index = 0) {
  const internalCat = (p.catSlug || (typeof p.category === 'string' ? p.category : null))
    ? normalizeCategoryId(p.catSlug || (typeof p.category === 'string' ? p.category : null))
    : null;
...
export function mapSanityPosts(posts = []) {
  return posts
    .filter((p) => p.slug?.current !== 'digital-assets-security')
    .map((p, i) => mapSanityPost(p, i));
}
