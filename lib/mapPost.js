...
export function mapSanityPost(p, index = 0) {
  const legacyTitle = p.categories?.[0];
  const internalCat = normalizeCategoryId(
    p.category || LEGACY_TITLE_TO_CATEGORY[legacyTitle] || legacyTitle
  );
  const catLabel = getCategoryLabel(internalCat);
