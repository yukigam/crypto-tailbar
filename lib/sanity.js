...
function buildCategoryCountsQuery() {
  return CATEGORY_IDS.map(
    (id) =>
      `"${id}": count(*[_type == "post" && (category == "${id}" || references(*[_type == "category" && slug.current == "${id}"]._id))])`
  ).join(',\n          ');
}
...
export async function getPostsByCategory(category) {
  ...
  try {
    return await client.fetch(
      `*[_type == "post" && (category == $category || references(*[_type == "category" && slug.current == $category]._id))] | order(_createdAt desc) {
        ${POST_FIELDS}
      }`,
      { category },
      fetchOptions
  ...
  try {
    return await client.fetch(
      `{
        ...
        "uncategorizedPosts": *[_type == "post" && !defined(category) && (!defined(categories) || count(categories) == 0)] | order(_createdAt desc) {
          ${POST_FIELDS}
        },
        "categoryCounts": {
...
export async function getCommunityPosts() {
  try {
    return await client.fetch(
      `*[_type == "post" && source == "community"] | order(_createdAt desc) {
        ${POST_FIELDS}
      }`,
      {},
      fetchOptions
