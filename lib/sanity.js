...
function buildCategoryCountsQuery() {
  return CATEGORY_IDS.map(
    (id) =>
      `"${id}": count(*[_type == "post" && isUserPost != true && (category == "${id}" || references(*[_type == "category" && slug.current == "${id}"]._id))])`
  ).join(',\n          ');
}
...
export async function getPostsByCategory(category) {
  ...
  try {
    return await client.fetch(
      `*[_type == "post" && isUserPost != true && (category == $category || references(*[_type == "category" && slug.current == $category]._id))] | order(_createdAt desc) {
        ${POST_FIELDS}
      }`,
      { category },
      fetchOptions
