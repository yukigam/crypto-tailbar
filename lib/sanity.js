...
function buildCategoryCountsQuery() {
  return CATEGORY_IDS.map(
    (id) =>
      `"${id}": count(*[_type == "post" && isUserPost != true && category == "${id}" && slug.current != "digital-assets-security"])`
  ).join(',\n          ');
}
...
export async function getPostsByCategory(category) {
  ...
  try {
    return await client.fetch(
      `*[_type == "post" && isUserPost != true && category == $category && slug.current != "digital-assets-security"] | order(_createdAt desc) {
        ${POST_FIELDS}
      }`,
      { category },
      fetchOptions
