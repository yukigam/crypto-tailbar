...
function buildCategoryCountsQuery() {
  return CATEGORY_IDS.map(
    (id) =>
      `"${id}": count(*[_type == "post" && isUserPost != true && defined(category) && category->slug.current == "${id}"])`
  ).join(',\n          ');
}
...
export async function getPostsByCategory(category) {
  ...
  try {
    return await client.fetch(
      `*[_type == "post" && isUserPost != true && defined(category) && category->slug.current == $category] | order(_createdAt desc) {
        ${POST_FIELDS}
      }`,
      { category },
      fetchOptions
