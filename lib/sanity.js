...
function buildCategoryCountsQuery() {
  return CATEGORY_IDS.map(
    (id) =>
      `"${id}": count(*[_type == "post" && isUserPost != true && category == "${id}" && !(slug.current match "digital-asset*")])`
  ).join(',\n          ');
}
...
export async function getPostsByCategory(category) {
  ...
  try {
    return await client.fetch(
      `*[_type == "post" && isUserPost != true && category == $category && !(slug.current match "digital-asset*")] | order(_createdAt desc) {
        ${POST_FIELDS}
      }`,
      { category },
      fetchOptions
...
export async function getPosts() {
  try {
    return await client.fetch(
      `{
        "posts": *[_type == "post" && isUserPost != true && !(slug.current match "digital-asset*")] | order(_createdAt desc) {
          ${POST_FIELDS}
        },
